...
- Used Goodreads API to retrieve and display ratings for all the books available in the bookstore
...
- Mailgun was configured with a domain name from NameCheap to send out order confirmation email to users to notify them that their payment is successful.

## Project Strategy and Scope
### User Stories
1. User would like to browse through bestsellers, new and trending as well as books that are under $5.00
Feature to implement: To include a carousel slider to display the bestsellers, new and trending as well as books that are under $5.00 to the user.
...
Feature to implement: To include an update shopping cart items function in the view shopping cart page that allows user to perform an update using AJAX jquery.
...
Feature to implement: To include a view purchase page to show the user the status of their order
...
Feature to implement: To include feature that allows only superuser to change a particular order status, i.e. from processing to either dispatched or delivered.

## Project Structure    
### a. Overview
This web application is structured into 8 main parts, i.e. user signing up for an account, logging in/out to an account, updating their user profile, browsing through the catalogues of books offered by the bookstore, add their favourite books to the shopping cart, update / delete their shopping cart items, checkout their shopping cart items as well as browse through their previous orders history.
...
At the main section of the landing page, users (both logged in as well as without logging in) will find a banner that display a short message informing the user that free shipping is applied for orders above $100 dollars and a Shop Now button below it. Further down, users will find three carousel sliders each displaying bestsellers book, new and trending book and also books under $5.00. Clicking on the books displayed on the carousel sliders will redirect the user to a show books detail page whereby user will be able to check out all the relevant information related to the particular book and also to add the item to the cart if they are currently logged in. At the very bottom of the page, user will be able to find a footer section that contains information about the bookstore itself, the customer services it provides, the get in touch with us, the find us on links as well as a form that allows user to subscribe to the bookstore's newsletter. 

Clicking on the Explore More Books tab on the navbar will redirect users to a page whereby they will be able to browse through all the books that are currently available in store. On the left side of the same page, users will be able to use the filter by function of the web application to filter out books of a particular genre, author or even price range. The query to fetch the results based on the filter option selected by the user will be done via AJAX and the results returned will be displayed using Django Digg-style pagination. To check out more on the books itself, the user will just have to click on the books of their choices and they will be redirected to the show books detail page.

Clicking on the Profile tab will redirect the users to the update user profile page. At this page, user will be able to see their firstname, lastname, username as well as email address. To update their user profile, they would just have to fill out the update user profile section and click on the update button.
...
Clicking on the View Cart tab will redirect the users to their shopping cart page whereby they will be able to see a breakdown of the items that are currently in their shopping cart. On the same page, users will be able delete a shopping cart item and also to update the quantity of books that they would like to order via AJAX. To checkout the items in the shopping cart, users will just need to click on the proceed to checkout button on the bottom right which then will redirect the user to a confirm checkout page that shows user a summary of their orders and the amount that they would have to pay for. Clicking on the Place Order button will then route the user to the checkout page whereby the user will need to provide their personal details as well as their credit card details in order to make a payment.
...
### a. Wireframes (need to modify)




### b. ER diagram (need to modify)
...
g. User are able to update the quantity of books to be ordered in the shopping cart page via AJAX.
...
l. Users are able to browse through their order history and also to check the status of each orders that has beend made.
m. Superuser are able to change the order status of a particular order, i.e. from processing to either dispatched or delivered.
...
## Project Surface (need to modify)
Design Choices:
...
## Technologies used (need to modify 13-15)
...
14. UploadCare was used to store the book images - https://uploadcare.com/
15. LucidChart was used to tool up the ER diagram for the database design - https://www.lucidchart.com/
16. W3C Markup was used to check the markup validity of HTML code - https://validator.w3.org/
17. JSHint was used to check check on the JS code - https://jshint.com/
18. Heroku was used to deploy the web application 
...
|18              | At the shopping cart page, user will be redirected to the home page of the website if they clicked on the continue shopping button. If they were to clicked on the proceed to checkout button, they will be redirected to the confirm checkout page. At the confirm checkout page, an order summary will be dispplayed showing users the details of their order and the amount that they would have to pay for. Clicking on the place order button will then redirect user to the checkout page whereby user will need to provide their personal and credit card details before being able to make a purchase. | Pass     |
...
a. Apps
    i. BookProject - This is the main project folder. The important files in this folder is the settings.py file and the urls.py file. Settings.py files typically contains all the settings configuration that is required to run the application. Urls.py file is the file that points to the different apps depending on the url that the user visited.
